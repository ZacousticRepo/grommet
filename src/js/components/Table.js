...
_onClick (event) {
      ...
      if (headerCells.length > 0) {
        ...
        [].forEach.call(rows, (row) => {
          ...
          [].forEach.call(row.cells, (cell) => {
            let headerCell =headerCells[headerIndex];
            if (headerCell)
              cell.setAttribute('data-th', (headerCell.innerText || headerCell.textContent));
            incrementCount++;
  ...
  _buildMirror () {
    ...
    if (tableElement && mirrorElement) {
      let row = tableElement.querySelectorAll('thead tr')[0];
      let cells = row.querySelectorAll('th');
      let mirrorRow = mirrorElement.querySelectorAll('thead tr')[0];
      let mirrorCells = mirrorRow.querySelectorAll('th');
      let previousElements = [];
      while (row.hasChildNodes()) {
        previousElements.push(row.removeChild(row.lastChild));
      }
      for (let i = 0; i < mirrorCells.length; i++) {
        const cell = mirrorCells[i];
        cell.style.width = '';
        cell.style.height = '';
  ...
  _alignMirror () {
    const { columnMode, rebuildMirror } = this.state;
    if (this.mirrorRef && !columnMode) {
      const tableElement = this.tableRef;
  ...
  render () {
    const {
      ...
      onMouseUp, responsive, scrollable, scrollableHeaders, selectable, ...props
    } = this.props;
    ...
    if (scrollable && !small) {
      mirror = (
                <table ref={ref => this.mirrorRef = ref} className={`${CLASS_ROOT}__mirror`}>
                <thead>
                <tr>{scrollableHeaders}</tr>
                </thead>
                </table>
                );
    }
    ...
    return (
            ...
            {mirror}
            <table ref={ref => this.tableRef = ref} {...selectableProps} className={tableClasses}>
            {scrollable && (<thead><tr></tr></thead>)}
            {children}
            </table>
            {more}
            </div>
...
Table.propTypes = {
  ...
  scrollableHeaders: PropTypes.arrayOf(PropTypes.object),
