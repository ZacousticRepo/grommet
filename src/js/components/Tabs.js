...
export default class Tabs extends Component {
  ...
  render () {
    const { children, className, justify, responsive, fixed, ...props } = this.props;
    ...
    let tabHeader;
    if (fixed) {
      tabHeader = (<Header fixed={true} responsive={responsive} style={{ position: 'fixed' }}>
                    <ul {...props} className={classes}>
                      {tabs}
                    </ul>
                  </Header>);
    } else {
      tabHeader = (<ul {...props} className={classes}>
                    {tabs}
                  </ul>);
    }

    return (
      <div role='tablist' className={`${CLASS_ROOT}--tablist`}>
        {tabHeader}
