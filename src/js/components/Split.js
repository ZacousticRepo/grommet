...
export default class Split extends Component {
  ...
  _layout () {
    ...
    if (splitElement) {
      if (splitElement.offsetWidth <= smallSize() && this.props.showOnResponsive === 'priority') {
  ...
  render () {
    const {
      children, className, fixed, flex, priority, separator, flexBlock, ...props
    } = this.props;
    ...
    const boxedChildren = !Array.isArray(children) ? children :
      children.map((child, index) => {
        ...
        const childProps = {};
        if (props.id) {
           childProps.id = (lastChild ? props.id + '-last' : props.id + '-' + index); 
        }
        let hidden;
        let childFlex = true;
        // When we only have room to show one child, hide the appropriate one
        if ('single' === responsive && 
              (('left' === priority && index > 0) || ('right' === priority && index === 0 && children.length > 1 && children[1])) ) 
        {
          hidden = true;
        ...
        const classes = classnames(
          `${CLASS_ROOT}__column`,
          {
            ...
            [`${CLASS_ROOT}__column--flex`]: (childFlex && !flexBlock),
            [`${CLASS_ROOT}__column--flex-block`]: (childFlex && flexBlock),
        ...
        return (
          <div key={index} {...childProps} className={classes}>
            {child}
          </div>
        );
      });
