...
//                    <svg className={`${CLASS_ROOT}__control-check`} viewBox='0 0 24 24' preserveAspectRatio='xMidYMid meet'>
...
export default class CheckBox extends Component
{
    render()
    {
        const { checked, className, disabled, label, name, onChange, reverse, toggle, ...props } = this.props;
        ...
        if (label)
        {
            labelNode = (<label key='label' htmlFor={props.id} className={`${CLASS_ROOT}__label`}>{label}</label>);
        }
