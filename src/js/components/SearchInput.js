...
export default class SearchInput extends Component {
  ...
  componentDidUpdate (prevProps, prevState) {
    const { suggestions, keepDropOpenOnSelection } = this.props;
    ...
    else if (dropActive && (prevState.dropActive || keepDropOpenOnSelection)) {
  ...
  _onEnter (event) {
    const { suggestions, onSelect, keepDropOpenOnSelection } = this.props;
    const { activeSuggestionIndex } = this.state;
    const { intl } = this.context;
    this.setState({ dropActive: !keepDropOpenOnSelection });
  ...
  _onClickSuggestion (suggestion) {
    const { onSelect, keepDropOpenOnSelection } = this.props;
    this.setState({value: suggestion, dropActive: !keepDropOpenOnSelection});
    if (onSelect) {
      onSelect({ target: this.inputRef, suggestion: suggestion });
    }
  }
  ...
  render () {
    ...
    const restProps = Props.omit(this.props, Object.keys(SearchInput.propTypes));

    return (
        <div {...restProps} ref={ref => this.componentRef = ref} className={classes}>
...
SearchInput.propTypes = {
  ...
  keepDropOpenOnSelection: PropTypes.bool,
